...
-- ИЗМЕНЕНИЕ: Уникальный индекс чтобы пользователь мог оставить только один ОДОБРЕННЫЙ отзыв на врача
-- Но может иметь несколько отзывов в статусе pending
CREATE UNIQUE INDEX IF NOT EXISTS idx_reviews_user_vet_approved_unique 
ON reviews(user_id, veterinarian_id) 
...
BEGIN
    ...
    IF user1_exists THEN
        ...
        ON CONFLICT (user_id, veterinarian_id) WHERE status = 'approved' DO NOTHING;
    END IF;
